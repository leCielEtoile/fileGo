...
- 🔒 **セキュアな設計** - パストラバーサル対策、セッション管理、監査ログ
...
# 2. Docker/サーバー系の環境変数を設定（認証情報は含めない）
cp .env.example .env
vi .env

# 3. 初回起動（config.yaml のひな型とディレクトリが自動生成される）
...
flowchart TB
    ...
    E["SQLite（database）<br/>users, sessions, access_logs, file_metadata, oidc_user_roles"]
    A --> B --> C --> D --> E
```
